function PPM.loadResources()
	...
	for k,v in ipairs(PPM.m_eyes)do
		if type(v)!="IMaterial"then
			timer.Simple(0,function()
				table.remove(PPM.m_eyes,k)
			end)
		elseif v:IsError() then
			PPM.m_eyes[k]=PPM.m_eyes[1]
		end
	end
	PPM.m_eye_grads={
	...
	for k,v in ipairs(PPM.m_eye_grads)do
		if type(v)!="IMaterial"then
			timer.Simple(0,function()
				table.remove(PPM.m_eye_grads,k)
			end)
		elseif v:IsError() then
			PPM.m_eye_grads[k]=PPM.m_eye_grads[1]
		end
	end
	...
	for k,v in ipairs(PPM.m_eye_reflections)do
		if type(v)!="IMaterial"then
			timer.Simple(0,function()
				table.remove(PPM.m_eye_reflections,k)
			end)
		elseif v:IsError() then
			PPM.m_eye_reflections[k]=PPM.m_eye_reflections[1]
		end
	end
	PPM.m_bodydetails = {
