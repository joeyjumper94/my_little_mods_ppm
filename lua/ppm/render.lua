if CLIENT then
	local ppm_render_always=true--CreateConVar("ppm_render_always","1",FCVAR_ARCHIVE,"decide if ponies should always be rendered"):GetBool()
	local ppm_render_distance=0--CreateConVar("ppm_render_distance","1000",FCVAR_ARCHIVE,"decide how close ponies have to be to render"):GetInt()
	function PPM:CanSee(ply,target,num)
		if ply==nil or target==nil then return false end
		if ppm_render_always then return true end
		if ply:GetShootPos():Distance(target:GetPos())>ppm_render_distance then return false end
		if util.TraceLine({start=ply:GetShootPos(),endpos=target:GetPos(),filter={ply,target}}).Hit and util.TraceLine({start=ply:GetShootPos(),endpos=target:EyePos(),filter={ply,target}}).Hit then return false end
		return true
	end
	...
	//PPM.bones_testDraw("pony_mature")
		...
		--if true then return end
		////////////////////RENDER
		for name,ent in ipairs(ents.GetAll()) do
			if ent:IsValid() and PPM.CanSee(nil,LocalPlayer(),ent) then
				if !ent:IsPlayer() then
					...
					if plyrag!=nil then
						if PPM.isValidPonyLight(plyrag) then
							if !PPM.isValidPony(plyrag) then--								plyrag.ponyCacheTarget=ent--								PPM.MarkData[plyrag]=PPM.MarkData[ent]
