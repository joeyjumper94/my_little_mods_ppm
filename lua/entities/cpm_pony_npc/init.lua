...
concommand.Add("ppm_spawn_pnpc",function(ply, tr)
	...
	if ( tobool( GetConVarNumber( "ppm_restrict_npc" )==1) and !ply:IsAdmin() ) then
		ply:PrintMessage( HUD_PRINTTALK, "the spawning of pony NPCs was restricted to admins and above." )
		return
	elseif ( tobool( GetConVarNumber( "ppm_restrict_npc")==2 ) and !ply:IsSuperAdmin() ) then
...
concommand.Add("ppm_spawn_pragdoll",function(ply, tr)
    if ( !IsValid( ply ) ) then return end
    if ( tobool( GetConVarNumber( "ppm_restrict_ragdoll" )==1) and !ply:IsAdmin() ) then
		ply:PrintMessage( HUD_PRINTTALK, "the spawning of pony ragdolls was restricted to admins and above." )
		return
	elseif ( tobool( GetConVarNumber( "ppm_restrict_ragdoll" )==2) and !ply:IsSuperAdmin() ) then
